...
- **Choropleth** of the 50 states + DC, recolored live by a metric toggle. Synthetic mode: available **hosting capacity (MW)**, median **queue wait (months)**, or active **queue volume (GW)**. Real mode: **proposed capacity (GW)**, **proposed projects**, or **queue pressure** (proposed ÷ existing). Each metric has its own dark-theme sequential ramp.
...
Honest scope: EIA-860M "planned" generators are a **subset** of the full interconnection queue — no withdrawn or early-study requests, no queue-entry date, and **no large loads / data centers** (that queue has no public project-level source). A true transmission "hosting capacity (MW)" per state isn't public either (FERC base cases are CEII-restricted), so the real choropleth is reframed as proposed capacity / queue pressure rather than headroom.
